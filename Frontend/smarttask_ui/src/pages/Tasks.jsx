...
const TaskCard = ({ task, onEdit, onDelete, onView, projects }) => {
  ...
  return (
    <div
      ...
      } ${isOverdue(task.due_date) ? 'border-red-200 bg-red-50/30' : ''}`}
    ...
    >
      <div className="p-6">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          {task.project && (
            <span className="text-xs text-gray-500 bg-gray-100 px-2.5 py-1 rounded-full flex items-center space-x-1">
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await updateTask(editingTask.id, {
        ...
        project: editingTask.project ? parseInt(editingTask.project) : null,
              ...
              <div className="flex items-center bg-white rounded-lg border border-gray-200 p-1">
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-sm transition-colors ${
                    viewMode === "grid" ? "bg-violet-100 text-violet-700" : "text-gray-500 hover:bg-gray-100"
                  }`}
                >
                  Grid
                </button>
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-sm transition-colors ${
                    viewMode === "list" ? "bg-violet-100 text-violet-700" : "text-gray-500 hover:bg-gray-100"
                  }`}
                >
                  List
                </button>
                      ...
                      sortedTasks.map((task) => {
                        ...
                        return (
                          <tr key={task.id} className={`hover:bg-gray-50 transition-colors ${isOverdue ? 'bg-red-50/30' : ''}`}>
                            ...
                            <td className="px-6 py-4">
                              <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${task.status === 'completed' ? 'bg-emerald-100 text-emerald-700' : task.status === 'in_progress' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}`}>
                                {task.status.replace('_', ' ')}
                              </span>
                            </td>
                            <td className="px-6 py-4">
                              <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${task.priority === 'high' ? 'bg-red-100 text-red-700' : task.priority === 'medium' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700'}`}>
                                {task.priority}
                              </span>
                            </td>
